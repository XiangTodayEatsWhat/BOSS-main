...
def parse_args():
    ...
    parser.add_argument('--nwpu', action='store_true', help='For LVIS models')
...
def copy_checkpoint(param_name, is_weight, checkpoint):
    """Either remove the final layer weights for fine-tuning on novel dataset
    or append randomly initialized weights for the novel classes.
    ...
    initialization step is not really necessary.
    """
    ends = '.weight' if is_weight else '.bias'
    weight_name = param_name + ends
    if weight_name in checkpoint['state_dict']:
        pretrained_weight = checkpoint['state_dict'][weight_name]
        checkpoint['state_dict']['{}.org_module{}'.format(param_name, ends)] = pretrained_weight
    ...
    if args.method == 'remove':
        ...
        for idx, (param_name,
            ...
            random_init_checkpoint(param_name, False, tar_size, checkpoint,
                                   args)
        for param_name in args.lora_name:
            copy_checkpoint(param_name, True, checkpoint)
            copy_checkpoint(param_name, False, checkpoint)
    else:
