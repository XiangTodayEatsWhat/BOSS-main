...
class LohaModule(nn.Module):
    ...
    def __init__(
        ...
        multiplier=1.0, lora_dim=8, alpha=4, 
    ...
    def forward(self, src_x, lora_x):
        ...
        src_weight = (self.org_module.weight.data.to(src_x.device, dtype=self.hada_w1_a.dtype))
        src_result = self.op(src_x, 
                             src_weight.view(self.shape),
                             bias,
                             **self.extra_args)
        
        lora_weight = (
            self.org_module.weight.data.to(lora_x.device, dtype=self.hada_w1_a.dtype)
            + self.get_weight(self.org_module.weight.data) * self.scalar * self.multiplier
        )
        lora_result = self.op(lora_x, 
                             lora_weight.view(self.shape),
                             bias,
                             **self.extra_args)
        
        bias = None if self.org_module.bias is None else self.org_module.bias.data
        return src_result, lora_result
    


class CombLohaModule(LohaModule):
    """
    Hadamard product Implementaion for Low Rank Adaptation
    """
    ...
    def forward(self, x):
        ...
        bias = None if self.org_module.bias is None else self.org_module.bias.data
        x = torch.cat([src_result, lora_result])
        return x